...
class iclk
{
		...
		public function iclk( )
		{
				...
				if ( $p )
				{
						...
						$qarr = explode( "|", $deurl );
						$this->siteurl = $GLOBALS['_GET']['z_c_url'] = $qarr[1];
						$this->referer = $GLOBALS['_GET']['z_uref'] = $qarr[2];
						$this->screen = $qarr[3];
						$arr = $this->screenwh( $this->screen );
						$GLOBALS['_GET']['z_sw'] = $arr[0];
						$GLOBALS['_GET']['z_sh'] = $arr[1];
						$GLOBALS['_GET']['z_scd'] = $arr[2];
						$this->timezone = $GLOBALS['_GET']['z_utz'] = ( integer )$qarr[4];
						$this->java = $GLOBALS['_GET']['z_ujava'] = ( integer )$qarr[5];
						$this->flash = $GLOBALS['_GET']['z_ufv'] = $qarr[6];
						$this->plugins = $GLOBALS['_GET']['z_unplug'] = $qarr[7];
						$this->mimetypes = $GLOBALS['_GET']['z_unmime'] = $qarr[8];
						$this->history = $GLOBALS['_GET']['z_uhis'] = ( integer )$qarr[9];
						$this->scrollheight = $GLOBALS['_GET']['z_uc_ks'] = $qarr[10];
						...
						{
								$marr = explode( ";", $m );
								$this->flash = $GLOBALS['_GET']['z_ufv'] = $marr[0];
								$this->screen = $marr[1];
								$arr = $this->screenwh( $this->screen );
								$GLOBALS['_GET']['z_sw'] = $arr[0];
								$GLOBALS['_GET']['z_sh'] = $arr[1];
								$GLOBALS['_GET']['z_scd'] = $arr[2];
								$this->siteurl = $GLOBALS['_GET']['z_c_url'] = $marr[2];
								$this->referer = $GLOBALS['_GET']['z_uref'] = $marr[3];
						}
						else
						{
		...
		public function run( )
		{
				...
				{
						$arr = explode( ",", $popads );
						if ( in_array( $this->adsid, $arr ) && 1 < count( $arr ) )
						{
								$hasbeenpop = TRUE;
						}
				...
				if ( $this->add_url == "2" || $hasbeenpop )
				{
						require_once( LIB_DIR."/cache/cache_".$GLOBALS['C_ZYIIS']['cache_type'].".php" );
						$GLOBALS['_GET']['zoneid'] = $this->zoneid;
						$GLOBALS['GLOBALS']['zone'] = gcmakecache( );
						$GLOBALS['GLOBALS']['ads'] = getlinkedad( $GLOBALS['zone'] );
						if ( $GLOBALS['ads'] )
						{
				...
				if ( !$this->zlink )
				{
						...
						$this->a = $this->getzone( );
						$advprice = ( double )$this->u['advprice'];
						if ( !empty( $advprice ) )
						{
		...
		public function redirect( $url = "" )
		{
				...
				$url = str_replace( array( "{uid}", "{adsid}", "{siteid}" ), array(
						$this->uid,
						$this->adsid,
						$this->siteid
				), $this->targeturl );
		...
		public function k9ovx1( $a, $u )
		{
				...
				else
				{
						...
						$nodosage = TRUE;
						if ( 0 < $a['dosage'] && 0 < $a['deduction'] && $deduction == 1 )
						{
